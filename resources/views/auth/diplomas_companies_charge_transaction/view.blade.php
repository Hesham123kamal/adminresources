...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.diplomas_companies_charge_transaction') }}</span>
            </li>
        </ul>
...
<style>
    ...
    @if(PerUser('diplomas_companies_charge_transaction_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/dcct/create') }}" id="sample_editable_1_new" class="btn green">
    ...
    @endif
    @if(PerUser('diplomas_companies_charge_transaction_view'))
        <div class="row">
            <div id="errors"></div>
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            {{--@if(PerUser('diplomas_companies_charge_transaction_publish'))--}}
            ...
            $(document).on('click','.add_all_courses',function(){
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ URL('admin/dcct/add_courses') }}/" + transaction_id,
                    data: { _token: token},
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/dcct_search') }}", // ajax source
                    },
            ...
            @if(PerUser('diplomas_companies_charge_transaction_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.diploma_company_charge_transaction') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.diploma_company_charge_transaction') }} ?',
                    buttons: [
                        {
            ...
            @endif
            @if(PerUser('diplomas_companies_charge_transaction_copy'))
            $(document).on('click', '.copy_this', function (event) {

                var copy_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.copy').' '.lang::get('main.diploma_company_charge_transaction') }}',
                    message: '{{ Lang::get('main.copy_this').lang::get('main.diploma_company_charge_transaction') }} ?',
                    buttons: [
                        {
                            label: '{{ Lang::get('main.yes') }}',
                            cssClass: 'btn-primary',
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "POST",
                                    url: "{{ URL('admin/dcct/copy') }}",
                                    data: {"id": copy_id, _token: token},
                                    success: function (msg) {
                                        window.location.href = "dcct";
                                    }
                                });
                            }
                        },
