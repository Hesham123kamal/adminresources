...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.recruitment_jobs_types') }}</span>
            </li>
        </ul>
    ...
    <!-- END PAGE BAR -->
    <!-- BEGIN PAGE TITLE-->
    <h1 class="page-title"> {{ Lang::get('main.recruitment_jobs_types') }}
...
<style>
    ...
    @if(PerUser('recruitment_jobs_types_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/recruitment_jobs_types/create') }}" id="sample_editable_1_new" class="btn green">
    ...
    @endif
    @if(PerUser('recruitment_jobs_types_view'))
        <div class="row">
            <div class="portlet light bordered">
                <div class="portlet-title">
                    <div class="caption font-dark">
                        <i class="icon-users font-dark"></i>
                        <span class="caption-subject bold uppercase">{{ Lang::get('main.recruitment_jobs_types') }}</span>
                    </div>
                    <div class="tools"></div>
                </div>
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            @if(PerUser('recruitment_jobs_types_publish'))
            $(document).on('change', '.changeStatues', function () {
                ...
                if (statues) {
                    $.ajax({
                        type: "POST",
                        url: "{{ URL('admin/recruitment_jobs_types/publish') }}",
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/recruitment_jobs_types_search') }}", // ajax source
                    },
            ...
            @if(PerUser('recruitment_jobs_types_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.recruitment_jobs_type') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.recruitment_jobs_type') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/recruitment_jobs_types') }}/" + deleted_id,
