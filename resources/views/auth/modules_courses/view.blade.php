...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.modules_courses') }}</span>
            </li>
        </ul>
    ...
    <!-- END PAGE BAR -->
    <!-- BEGIN PAGE TITLE-->
    <h1 class="page-title"> {{ Lang::get('main.modules_courses') }}
...
@section('content')
    ...
    @if(PerUser('modules_courses_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/modules_courses/create') }}" id="sample_editable_1_new" class="btn green">
    ...
    @endif
    @if(PerUser('modules_courses_view'))
        <div class="row">
            <div class="portlet light bordered">
                <div class="portlet-title">
                    <div class="caption font-dark">
                        <i class="icon-users font-dark"></i>
                        <span class="caption-subject bold uppercase">{{ Lang::get('main.modules_courses') }}</span>
                    </div>
                    <div class="tools"></div>
                </div>
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            @if(PerUser('modules_courses_active'))
            $(document).on('change', '.changeStatues', function () {
                ...
                if (statues) {
                    $.ajax({
                        type: "POST",
                        url: "{{ URL('admin/modules_courses/activation') }}",
            ...
            grid.init({
                ...
                onDataLoad: function (grid) {
                    ...
                    {{--$edit_drug.editable({--}}
                        {{--source: [--}}
                            ...
                            {{--}--}}
                        {{--],--}}
                        {{--url: "{{ URL('admin/modules_courses') }}/" + $edit_drug_status_id + '/editsuccessstoriestatus',--}}
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/modules_courses_search') }}", // ajax source
                    },
            ...
            $(document).on('click', '.confirm_deletion', function (event) {
                ...
                $.ajax(
                    {
                        url: "{{ URL('admin/modules_courses') }}/" + drug_id,
            ...
            @if(PerUser('modules_courses_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.modules_courses') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.modules_courses') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/modules_courses') }}/" + deleted_id,
