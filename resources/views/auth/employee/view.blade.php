...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.employees') }}</span>
            </li>
        </ul>
    ...
    <!-- END PAGE BAR -->
    <!-- BEGIN PAGE TITLE-->
    <h1 class="page-title"> {{ Lang::get('main.employees') }}
...
<style>
    ...
    @if(PerUser('employee_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/employee/create') }}" id="sample_editable_1_new" class="btn green">
    ...
    @endif
    @if(PerUser('employee_view'))
        <div class="row">
            <div class="portlet light bordered">
                <div class="portlet-title">
                    <div class="caption font-dark">
                        <i class="icon-users font-dark"></i>
                        <span class="caption-subject bold uppercase">{{ Lang::get('main.employees') }}</span>
                    </div>
                    <div class="tools"></div>
                </div>
                <div class="portlet-body">
                    <table style=" table-layout: fixed;width: 100%;" class="table table-striped table-bordered table-hover table-checkable" width="100%"
                        ...
                        <tr role="row" class="heading">
                            <th> #</th>
                            <th>{{ Lang::get('main.username') }}</th>
                            <th>{{ Lang::get('main.password') }}</th>
                            <th>{{ Lang::get('main.user_level') }}</th>
                            <th>{{ Lang::get('main.support_level') }}</th>
                        ...
                        <tr role="row" class="filter">
                            ...
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="password">
                            </td>
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="userLevel">
                            </td>
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="support_level">
                            </td>
                            <td>
        ...
        $(document).ready(function () {
            ...
            $(document).on('change', '.changeStatues', function () {
                ...
                if (statues) {
                    $.ajax({
                        type: "POST",
                        url: "{{ URL('admin/employee/publish') }}",
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/employee_search') }}", // ajax source
                    },
            ...
            @if(PerUser('employee_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.employee') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.employee') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/employee') }}/" + deleted_id,
