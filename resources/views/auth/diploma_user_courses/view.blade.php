...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.diploma_user_courses') }}</span>
            </li>
        </ul>
...
<style>
    ...
    @if(PerUser('diploma_user_courses_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/diploma_user_courses/create') }}" id="sample_editable_1_new" class="btn green">
    ...
    @endif
    @if(PerUser('diploma_user_courses_view'))
        <div class="row">
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            {{--@if(PerUser('diploma_user_courses_publish'))--}}
            {{--$(document).on('change', '.changeStatues', function () {--}}
                ...
                {{--if (statues) {--}}
                    {{--$.ajax({--}}
                        {{--type: "POST",--}}
                        {{--url: "{{ URL('admin/diploma_user_courses/publish') }}",--}}
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/diploma_user_courses_search') }}", // ajax source
                    },
            ...
            @if(PerUser('diploma_user_courses_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.diploma_user_courses') }}',
                    ...
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/diploma_user_courses') }}/" + deleted_id,
            ...
            @endif
             ...
             @if(PerUser('diploma_user_courses_delete_all'))
            $(document).on('click', '.delete_all_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete_all').lang::get('main.diploma_user_courses') }}',
                    message: '{{ Lang::get('main.delete_all').lang::get('main.diploma_user_courses').lang::get('main.for_this_diploma') }} ?',
                    buttons: [
                        {
                            label: '{{ Lang::get('main.yes') }}',
                            cssClass: 'btn-primary',
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "POST",
                                    url: "{{ URL('admin/diploma_user_courses/delete_all') }}",
                                    data: {"id": deleted_id, _token: token},
                                    success: function (msg) {
                                        dialogItself.close();
                                        location.reload();
