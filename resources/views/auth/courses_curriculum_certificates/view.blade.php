...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.courses_curriculum_certificates') }}</span>
            </li>
        </ul>
...
<style>
    ...
    @if(PerUser('courses_curriculum_certificates_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/courses_curriculum_certificates/create') }}" id="sample_editable_1_new" class="btn green">
    ...
    @endif
    @if(PerUser('courses_curriculum_certificates_view'))
        <div class="row">
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            @if(PerUser('courses_curriculum_certificates_publish'))
            $(document).on('change', '.changeStatues', function () {
                ...
                if (statues) {
                    $.ajax({
                        type: "POST",
                        url: "{{ URL('admin/courses_curriculum_certificates/publish') }}",
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/courses_curriculum_certificates_search') }}", // ajax source
                    },
            ...
            @if(PerUser('courses_curriculum_certificates_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.course_curriculum_certificate') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.course_curriculum_certificate') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/courses_curriculum_certificates') }}/" + deleted_id,
            ...
            @endif
            @if(PerUser('courses_curriculum_certificates_copy'))
            $(document).on('click', '.copy_this', function (event) {

                var copy_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.copy').' '.lang::get('main.course_curriculum_certificate') }}',
                    message: '{{ Lang::get('main.copy_this').lang::get('main.course_curriculum_certificate') }} ?',
                    buttons: [
                        {
                            label: '{{ Lang::get('main.yes') }}',
                            cssClass: 'btn-primary',
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "POST",
                                    url: "{{ URL('admin/courses_curriculum_certificates/copy') }}",
                                    data: {"id": copy_id, _token: token},
                                    success: function (msg) {
                                        window.location.href = "courses_curriculum_certificates";
                                    }
                                });
                            }
                        },
