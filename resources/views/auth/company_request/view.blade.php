...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.company_request') }}</span>
            </li>
        </ul>
...
@if(PerUser('company_request_add'))
    <div class="row">
        <div class="col-md-12">
            <div class="btn-group pull-right">
                <a href="{{ URL('admin/company_request/create') }}" id="sample_editable_1_new" class="btn green">
...
@endif
    @if(PerUser('company_request_view'))
        <div class="row">
                ...
                <div class="portlet-body">
                    <table style=" table-layout: fixed;width: 100%;" class="table table-striped table-bordered table-hover table-checkable" width="100%"
                        ...
                        <tr role="row" class="filter">
                            ...
                            <td>
                                <input type="number" min="1" class="form-control form-filter input-sm" name="user_id">
                            </td>
                            <td>
                            ...
                            <td>
                                {{--<select name="user" class="module_name sel2 form-control form-filter"--}}
                                        {{--id="user">--}}
                            ...
                            <td>
                                <select name="type" class="module_name sel2 form-control form-filter">
                                    ...
                                    <option value="default">{{ Lang::get('main.default') }}</option>
                                    <option value="lite_version">{{ Lang::get('main.lite_version') }}</option>
                                </select>
                            </td>
                            <td>
        ...
        $(document).ready(function () {
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/company_request_search') }}", // ajax source
                    },
            ...
            @if(PerUser('company_request_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.company_request') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.company_request') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/company_request') }}/" + deleted_id,
                                    data: {"id": deleted_id, _token:"{{ csrf_token() }}" },
