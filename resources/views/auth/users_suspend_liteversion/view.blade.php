...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.users_suspend_liteversion') }}</span>
            </li>
        </ul>
...
<style>
    ...
    @if(PerUser('users_suspend_liteversion_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/users_suspend_liteversion/create') }}" id="sample_editable_1_new" class="btn green">
    ...
    @endif
    @if(PerUser('users_suspend_liteversion_view'))
        <div class="row">
                ...
                <div class="portlet-body">
                    <table style=" table-layout: fixed;width: 100%;" class="table table-striped table-bordered table-hover table-checkable" width="100%"
                        ...
                        <tr role="row" class="heading">
                            ...
                            <th>{{ Lang::get('main.suspendtime') }}</th>
                        ...
                        <tr role="row" class="filter">
                            ...
                            <td>
                                <select name="type" class="module_name sel2 form-control form-filter">
                                    ...
                                    <option value="suspend">{{ Lang::get('main.suspend') }}</option>
                                    <option value="unsuspend">{{ Lang::get('main.unsuspend') }}</option>
                                </select>
                            </td>
                            <td>
                                <div class="input-group date fromToDate margin-bottom-5" data-date-format="yyyy-mm-dd">
                                    <input type="text" class="form-control form-filter input-sm"
                                           name="suspendtime_from" placeholder="{{ Lang::get('main.from') }}">
                                ...
                                <div class="input-group date fromToDate" data-date-format="yyyy-mm-dd">
                                    <input type="text" class="form-control form-filter input-sm"
                                           name="suspendtime_to" placeholder="{{ Lang::get('main.to') }}">
                                    <span class="input-group-btn">
                                            <button class="btn btn-sm default" type="button">
                                                <i class="fa fa-calendar"></i>
                                            </button>
        ...
        $(document).ready(function () {
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/users_suspend_liteversion_search') }}", // ajax source
                    },
            ...
            @if(PerUser('users_suspend_liteversion_delete'))
            $(document).on('click', '.delete_this', function (users_suspend_liteversion) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.users_suspend_liteversion') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.users_suspend_liteversion') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/users_suspend_liteversion') }}/" + deleted_id,
