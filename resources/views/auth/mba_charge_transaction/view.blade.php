...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.mba_charge_transactions') }}</span>
            </li>
        </ul>
...
<style>
    ...
    @if(PerUser('mba_charge_transaction_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/mba_charge_transaction/create') }}" id="sample_editable_1_new" class="btn green">
    ...
    @endif
    @if(PerUser('mba_charge_transaction_view'))
        <div class="row">
            <div class="portlet light bordered">
                <div class="portlet-title">
                    <div class="caption font-dark">
                        <i class="icon-users font-dark"></i>
                        <span class="caption-subject bold uppercase">{{ Lang::get('main.mba_charge_transaction') }}</span>
                    </div>
                    <div class="tools"></div>
                </div>
                <div class="portlet-body">
                    <table style=" table-layout: fixed;width: 100%;" class="table table-striped table-bordered table-hover table-checkable" width="100%"
                        ...
                        <tr role="row" class="heading">
                            <th> #</th>
{{--                            <th>{{ Lang::get('main.mba_name') }}</th>--}}
                            <th>{{ Lang::get('main.mba_price') }}</th>
...
{{--                            <th>{{ Lang::get('main.published') }}</th>--}}
                        ...
                        <tr role="row" class="filter">
                            ...
                            {{--</td>--}}
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="mba_price">
                            </td>
                            <td>
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            {{--@if(PerUser('mba_charge_transaction_publish'))--}}
            {{--$(document).on('change', '.changeStatues', function () {--}}
                ...
                {{--if (statues) {--}}
                    {{--$.ajax({--}}
                        {{--type: "POST",--}}
                        {{--url: "{{ URL('admin/mba_charge_transaction/publish') }}",--}}
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/mba_charge_transaction_search') }}", // ajax source
                    },
            ...
            @if(PerUser('mba_charge_transaction_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.mba_charge_transaction') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.mba_charge_transaction') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/mba_charge_transaction') }}/" + deleted_id,
            ...
            @endif
            @if(PerUser('mba_charge_transaction_copy'))
            $(document).on('click', '.copy_this', function (event) {

                var copy_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.copy').' '.lang::get('main.mba_charge_transaction') }}',
                    message: '{{ Lang::get('main.copy_this').lang::get('main.mba_charge_transaction') }} ?',
                    buttons: [
                        {
                            label: '{{ Lang::get('main.yes') }}',
                            cssClass: 'btn-primary',
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "POST",
                                    url: "{{ URL('admin/mba_charge_transaction/copy') }}",
                                    data: {"id": copy_id, _token: token},
                                    success: function (msg) {
                                        window.location.href = "mba_charge_transaction";
                                    }
                                });
                            }
                        },
