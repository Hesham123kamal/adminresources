...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.diploma_certificate') }}</span>
            </li>
        </ul>
    ...
    <!-- END PAGE BAR -->
    <!-- BEGIN PAGE TITLE-->
    <h1 class="page-title"> {{ Lang::get('main.diploma_certificate') }}
...
<style>
    ...
    @if(PerUser('diploma_certificates_view'))
        <div class="row">
            <div class="portlet light bordered">
                <div class="portlet-title">
                    <div class="caption font-dark">
                        <i class="icon-users font-dark"></i>
                        <span class="caption-subject bold uppercase">{{ Lang::get('main.diploma_certificate') }}</span>
                    </div>
                    <div class="tools"></div>
                </div>
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            {{--@if(PerUser('diploma_certificates_publish'))--}}
            {{--$(document).on('change', '.changeStatues', function () {--}}
                ...
                {{--if (statues) {--}}
                    {{--$.ajax({--}}
                        {{--type: "POST",--}}
                        {{--url: "{{ URL('admin/diploma_certificates/publish') }}",--}}
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/diploma_certificates_search') }}", // ajax source
                    },
            ...
            @if(PerUser('diploma_certificates_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.diploma_certificate') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.diploma_certificate') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/diploma_certificates') }}/" + deleted_id,
            ...
            @endif
            @if(PerUser('diploma_certificates_copy'))
            $(document).on('click', '.copy_this', function (event) {

                var copy_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.copy').' '.lang::get('main.diploma_certificate') }}',
                    message: '{{ Lang::get('main.copy_this').lang::get('main.diploma_certificate') }} ?',
                    buttons: [
                        {
                            label: '{{ Lang::get('main.yes') }}',
                            cssClass: 'btn-primary',
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "POST",
                                    url: "{{ URL('admin/diploma_certificates/copy') }}",
                                    data: {"id": copy_id, _token: token},
                                    success: function (msg) {
                                        window.location.href = "diploma_certificates";
                                    }
                                });
                            }
                        },
