...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.medical_sup_categories') }}</span>
            </li>
        </ul>
...
@section('content')
    ...
    @if(PerUser('medical_sup_categories_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/medical_sup_categories/create') }}" id="sample_editable_1_new" class="btn green">
    ...
    @endif
    @if(PerUser('medical_sup_categories_view'))
        <div class="row">
                ...
                <div class="portlet-body">
                    <table class="table table-striped table-bordered table-hover dt-responsive" width="100%"
                        ...
                        <tr role="row" class="heading">
                            <th width="8%"> #</th>
                            <th width="20%">{{ Lang::get('main.category_name') }}</th>
                            <th width="20%">{{ Lang::get('main.name') }}</th>
                            <th width="10%">{{ Lang::get('main.price') }}</th>
                            <th width="10%">{{ Lang::get('main.url') }}</th>
                            <th width="10%">{{ Lang::get('main.image') }}</th>
                            <th>{{ Lang::get('main.courses') }}</th>
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            @if(PerUser('medical_sup_categories_publish'))
            $(document).on('change', '.changeStatues', function () {
                ...
                if (statues) {
                    $.ajax({
                        type: "POST",
                        url: "{{ URL('admin/medical_sup_categories/publish') }}",
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/medical_sup_categories_search') }}", // ajax source
                    },
            ...
            {{--$(document).on('click', '.confirm_deletion', function (event) {--}}
                ...
                {{--$.ajax(--}}
                    {{--{--}}
                        {{--url: "{{ URL('admin/medical_sup_categories') }}/" + drug_id,--}}
            ...
            {{--});--}}
            @if(PerUser('medical_sup_categories_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.medical_category') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.medical_category') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/medical_sup_categories') }}/" + deleted_id,
