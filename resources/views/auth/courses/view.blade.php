...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.courses') }}</span>
            </li>
        </ul>
...
<style>
    ...
    @if(PerUser('courses_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/courses/create') }}" id="sample_editable_1_new" class="btn green">
                ...
                <div class="portlet-body">
                    <table style=" table-layout: fixed;width: 100%;" class="table table-striped table-bordered table-hover table-checkable" width="100%"
                        ...
                        <tr role="row" class="heading">
                            ...
                            <th>{{ Lang::get('main.en_name') }}</th>
                            <th>{{ Lang::get('main.rating') }}</th>
                            <th>{{ Lang::get('main.rating_count') }}</th>
                            <th>{{ Lang::get('main.view') }}</th>
                            <th>{{ Lang::get('main.sent') }}</th>
                            <th>{{ Lang::get('main.certificate_increment') }}</th>
                            <th>{{ Lang::get('main.show_on') }}</th>
        ...
        $(document).ready(function () {
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/courses_search') }}", // ajax source
                    },
            ...
            @if(PerUser('courses_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.course') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.course') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/courses') }}/" + deleted_id,
