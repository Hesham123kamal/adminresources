...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.medical_categories_charge_transactions') }}</span>
            </li>
        </ul>
    ...
    <!-- END PAGE BAR -->
    <!-- BEGIN PAGE TITLE-->
    <h1 class="page-title"> {{ Lang::get('main.medical_categories_charge_transactions') }}
...
<style>
    ...
    @if(PerUser('medical_categories_charge_transactions_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/medical_categories_charge_transactions/create') }}" id="sample_editable_1_new" class="btn green">
    ...
    @endif
    @if(PerUser('medical_categories_charge_transactions_view'))
        <div class="row">
            <div class="portlet light bordered">
                <div class="portlet-title">
                    <div class="caption font-dark">
                        <i class="icon-users font-dark"></i>
                        <span class="caption-subject bold uppercase">{{ Lang::get('main.medical_categories_charge_transactions') }}</span>
                    </div>
                    <div class="tools"></div>
                </div>
                <div class="portlet-body">
                    <table style=" table-layout: fixed;width: 100%;" class="table table-striped table-bordered table-hover table-checkable" width="100%"
                        ...
                        <tr role="row" class="heading">
                            ...
                            <th>{{ Lang::get('main.amount') }}</th>
                            <th>{{ Lang::get('main.currency') }}</th>
                            <th>{{ Lang::get('main.period') }}</th>
                            <th>{{ Lang::get('main.start_date') }}</th>
                            <th>{{ Lang::get('main.end_date') }}</th>
                            <th>{{ Lang::get('main.pending') }}</th>
                            <th>{{ Lang::get('main.subscribe_type') }}</th>
                            <th>{{ Lang::get('main.created_time') }}</th>
                            <th>{{ Lang::get('main.all_category') }}</th>
                            <th></th>
                        </tr>
                        <tr role="row" class="filter">
                            ...
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="currency">
                            </td>
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="period">
                            </td>
                            <td>
                                <div class="input-group date fromToDate margin-bottom-5" data-date-format="yyyy-mm-dd">
                                    <input type="text" class="form-control form-filter input-sm"
                                           name="start_date_from" placeholder="{{ Lang::get('main.from') }}">
                                ...
                                <div class="input-group date fromToDate" data-date-format="yyyy-mm-dd">
                                    <input type="text" class="form-control form-filter input-sm"
                                           name="start_date_to" placeholder="{{ Lang::get('main.to') }}">
                                    <span class="input-group-btn">
                                            <button class="btn btn-sm default" type="button">
                                                <i class="fa fa-calendar"></i>
                                            </button>
                            ...
                            <td>
                                <div class="input-group date fromToDate margin-bottom-5" data-date-format="yyyy-mm-dd">
                                    <input type="text" class="form-control form-filter input-sm"
                                           name="end_date_from" placeholder="{{ Lang::get('main.from') }}">
                                    <span class="input-group-btn">
                                            <button class="btn btn-sm default" type="button">
                                                <i class="fa fa-calendar"></i>
                                            </button>
                                ...
                                <div class="input-group date fromToDate" data-date-format="yyyy-mm-dd">
                                    <input type="text" class="form-control form-filter input-sm"
                                           name="end_date_to" placeholder="{{ Lang::get('main.to') }}">
                                    <span class="input-group-btn">
                                            <button class="btn btn-sm default" type="button">
                                                <i class="fa fa-calendar"></i>
                                            </button>
                            ...
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="pending">
                            </td>
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="subscribe_type">
                            </td>
                            <td>
                                <div class="input-group date fromToDate margin-bottom-5" data-date-format="yyyy-mm-dd">
                                    <input type="text" class="form-control form-filter input-sm"
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            @if(PerUser('medical_categories_charge_transactions_publish'))
            $(document).on('change', '.changeStatues', function () {
                ...
                if (statues) {
                    $.ajax({
                        type: "POST",
                        url: "{{ URL('admin/medical_categories_charge_transactions/publish') }}",
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/medical_categories_charge_transactions_search') }}", // ajax source
                    },
            ...
            @if(PerUser('medical_categories_charge_transactions_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.charge_transaction') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.charge_transaction') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/medical_categories_charge_transactions') }}/" + deleted_id,
            ...
            @endif
            @if(PerUser('medical_categories_charge_transactions_copy'))
            $(document).on('click', '.copy_this', function (event) {

                var copy_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.copy').' '.lang::get('main.charge_transaction') }}',
                    message: '{{ Lang::get('main.copy_this').lang::get('main.charge_transaction') }} ?',
                    buttons: [
                        {
                            label: '{{ Lang::get('main.yes') }}',
                            cssClass: 'btn-primary',
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "POST",
                                    url: "{{ URL('admin/medical_categories_charge_transactions/copy') }}",
                                    data: {"id": copy_id, _token: token},
                                    success: function (msg) {
                                        window.location.href = "medical_categories_charge_transactions";
                                    }
                                });
                            }
                        },
