...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.international_diplomas_charge_transaction') }}</span>
            </li>
        </ul>
    ...
    <!-- END PAGE BAR -->
    <!-- BEGIN PAGE TITLE-->
    <h1 class="page-title"> {{ Lang::get('main.international_diplomas_charge_transaction') }}
...
<style>
    ...
    @if(PerUser('international_diplomas_charge_transaction_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/idct/create') }}" id="sample_editable_1_new" class="btn green">
    ...
    @endif
    @if(PerUser('international_diplomas_charge_transaction_view'))
        <div class="row">
            <div id="errors"></div>
            <div class="portlet light bordered">
                <div class="portlet-title">
                    <div class="caption font-dark">
                        ...
                        <span class="caption-subject bold uppercase">{{ Lang::get('main.international_diplomas_charge_transaction') }}</span>
                    </div>
                    <div class="tools"></div>
                </div>
                <div class="portlet-body">
                    <table style=" table-layout: fixed;width: 100%;" class="table table-striped table-bordered table-hover table-checkable" width="100%"
                        ...
                        <tr role="row" class="heading">
                            ...
                            <th>{{ Lang::get('main.suspend') }}</th>
                            <th>{{ Lang::get('main.suspend_date') }}</th>
{{--                            <th>{{ Lang::get('main.published') }}</th>--}}
                        ...
                        <tr role="row" class="filter">
                            ...
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="suspend">
                            </td>
                            <td>
                                <div class="input-group date fromToDate margin-bottom-5" data-date-format="yyyy-mm-dd">
                                    <input type="text" class="form-control form-filter input-sm"
                                           name="suspend_date_from" placeholder="{{ Lang::get('main.from') }}">
                                    <span class="input-group-btn">
                                            <button class="btn btn-sm default" type="button">
                                                <i class="fa fa-calendar"></i>
                                            </button>
                                ...
                                <div class="input-group date fromToDate" data-date-format="yyyy-mm-dd">
                                    <input type="text" class="form-control form-filter input-sm"
                                           name="suspend_date_to" placeholder="{{ Lang::get('main.to') }}">
                                    <span class="input-group-btn">
                                            <button class="btn btn-sm default" type="button">
                                                <i class="fa fa-calendar"></i>
                                            </button>
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            {{--@if(PerUser('international_diplomas_charge_transaction_publish'))--}}
            ...
            {{--});--}}
            {{--@endif--}}
            {{--$(document).on('click','.add_all_courses',function(){--}}
                {{--transaction_id=$(this).attr('data-id')--}}
                {{--$.ajax({--}}
                    {{--type: "POST",--}}
                    {{--url: "{{ URL('admin/idct/add_courses') }}/" + transaction_id,--}}
                    {{--data: { _token: token},--}}
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/idct_search') }}", // ajax source
                    },
            ...
            @if(PerUser('international_diplomas_charge_transaction_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.international_diploma_charge_transaction') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.international_diploma_charge_transaction') }} ?',
                    buttons: [
                        {
            ...
            @endif
            @if(PerUser('international_diplomas_charge_transaction_copy'))
            $(document).on('click', '.copy_this', function (event) {

                var copy_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.copy').' '.lang::get('main.international_diploma_charge_transaction') }}',
                    message: '{{ Lang::get('main.copy_this').lang::get('main.international_diploma_charge_transaction') }} ?',
                    buttons: [
                        {
                            label: '{{ Lang::get('main.yes') }}',
                            cssClass: 'btn-primary',
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "POST",
                                    url: "{{ URL('admin/idct/copy') }}",
                                    data: {"id": copy_id, _token: token},
                                    success: function (msg) {
                                        window.location.href = "idct";
                                    }
                                });
                            }
                        },
