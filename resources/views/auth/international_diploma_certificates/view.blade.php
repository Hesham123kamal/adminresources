...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.international_diploma_certificate') }}</span>
            </li>
        </ul>
    ...
    <!-- END PAGE BAR -->
    <!-- BEGIN PAGE TITLE-->
    <h1 class="page-title"> {{ Lang::get('main.international_diploma_certificate') }}
...
<style>
    ...
    @if(PerUser('international_diploma_certificates_view'))
        <div class="row">
            <div class="portlet light bordered">
                <div class="portlet-title">
                    <div class="caption font-dark">
                        <i class="icon-users font-dark"></i>
                        <span class="caption-subject bold uppercase">{{ Lang::get('main.international_diploma_certificate') }}</span>
                    </div>
                    <div class="tools"></div>
                </div>
                <div class="portlet-body">
                    <table style=" table-layout: fixed;width: 100%;" class="table table-striped table-bordered table-hover table-checkable" width="100%"
                        ...
                        <tr role="row" class="heading">
                            <th> #</th>
                            <th>{{ Lang::get('main.serial_number') }}</th>
                            <th>{{ Lang::get('main.user_name') }}</th>
                            <th>{{ Lang::get('main.user_name_en') }}</th>
...
{{--                            <th>{{ Lang::get('main.published') }}</th>--}}
                        ...
                        <tr role="row" class="filter">
                            ...
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="user_name_en">
                            </td>
                            <td>
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            {{--@if(PerUser('international_diploma_certificates_publish'))--}}
            {{--$(document).on('change', '.changeStatues', function () {--}}
                ...
                {{--if (statues) {--}}
                    {{--$.ajax({--}}
                        {{--type: "POST",--}}
                        {{--url: "{{ URL('admin/international_diploma_certificates/publish') }}",--}}
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/international_diploma_certificates_search') }}", // ajax source
                    },
            ...
            @if(PerUser('international_diploma_certificates_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.international_diploma_certificate') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.international_diploma_certificate') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/international_diploma_certificates') }}/" + deleted_id,
            ...
            @endif
            @if(PerUser('international_diploma_certificates_copy'))
            $(document).on('click', '.copy_this', function (event) {

                var copy_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.copy').' '.lang::get('main.international_diploma_certificate') }}',
                    message: '{{ Lang::get('main.copy_this').lang::get('main.international_diploma_certificate') }} ?',
                    buttons: [
                        {
                            label: '{{ Lang::get('main.yes') }}',
                            cssClass: 'btn-primary',
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "POST",
                                    url: "{{ URL('admin/international_diploma_certificates/copy') }}",
                                    data: {"id": copy_id, _token: token},
                                    success: function (msg) {
                                        window.location.href = "international_diploma_certificates";
                                    }
                                });
                            }
                        },
