...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.support_transaction') }}</span>
            </li>
        </ul>
...
<style>
    ...
    @if(PerUser('support_transaction_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/support_transaction/create') }}" id="sample_editable_1_new" class="btn green">
    ...
    @endif
    @if(PerUser('support_transaction_view'))
        <div class="row">
                ...
                <div class="portlet-body">
                    <table style=" table-layout: fixed;width: 100%;" class="table table-striped table-bordered table-hover table-checkable" width="100%"
                        ...
                        <tr role="row" class="heading">
                            ...
                            <th>{{ Lang::get('main.old_email') }}</th>
                            <th>{{ Lang::get('main.new_email') }}</th>
                            <th>{{ Lang::get('main.old_phone') }}</th>
                            <th>{{ Lang::get('main.new_phone') }}</th>
                        ...
                        <tr role="row" class="filter">
                            ...
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="old_email">
                            </td>
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="new_email">
                            </td>
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="old_phone">
                            </td>
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="new_phone">
                            </td>
                            <td>
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            {{--@if(PerUser('support_transaction_publish'))--}}
            {{--$(document).on('change', '.changeStatues', function () {--}}
                ...
                {{--if (statues) {--}}
                    {{--$.ajax({--}}
                        {{--type: "POST",--}}
                        {{--url: "{{ URL('admin/support_transaction/publish') }}",--}}
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/support_transaction_search') }}", // ajax source
                    },
            ...
            @if(PerUser('support_transaction_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.support_transaction') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.support_transaction') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/support_transaction') }}/" + deleted_id,
