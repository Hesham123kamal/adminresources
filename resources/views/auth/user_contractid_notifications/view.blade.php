...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.user_contractid_notifications') }}</span>
            </li>
        </ul>
...
<style>
    ...
    @if(PerUser('user_contractid_notifications_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/user_contractid_notifications/create') }}" id="sample_editable_1_new" class="btn green">
    ...
    @endif
    @if(PerUser('user_contractid_notifications_view'))
        <div class="row">
                ...
                <div class="portlet-body">
                    <table style=" table-layout: fixed;width: 100%;" class="table table-striped table-bordered table-hover table-checkable" width="100%"
                        ...
                        <tr role="row" class="heading">
                            ...
                            <th>{{ Lang::get('main.subscription_contractid') }}</th>
                            <th>{{ Lang::get('main.customer_account_number') }}</th>
                            <th>{{ Lang::get('main.status') }}</th>
                            <th>{{ Lang::get('main.digest') }}</th>
                        ...
                        <tr role="row" class="filter">
                            ...
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="act">
                            </td>
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="subscription_contractid">
                            </td>
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="customer_account_number">
                            </td>
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="status">
                            </td>
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="digest">
                            </td>
                            <td>
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            {{--@if(PerUser('user_contractid_notifications_publish'))--}}
            {{--$(document).on('change', '.changeStatues', function () {--}}
                ...
                {{--if (statues) {--}}
                    {{--$.ajax({--}}
                        {{--type: "POST",--}}
                        {{--url: "{{ URL('admin/user_contractid_notifications/publish') }}",--}}
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/user_contractid_notifications_search') }}", // ajax source
                    },
            ...
            @if(PerUser('user_contractid_notifications_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.user_contractid_notifications') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.user_contractid_notifications') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/user_contractid_notifications') }}/" + deleted_id,
