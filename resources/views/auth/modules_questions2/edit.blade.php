...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <a href="{{ URL('/admin/modules_questions') }}">{{ Lang::get('main.modules_questions') }}</a>
                <i class="fa fa-circle"></i>
            </li>
            <li>
...
<div class="row">
    <div class="portlet light col-md-6 bordered">
        ...
        <div class="portlet-body" style="overflow-y: auto;">
            {!! Form::open(['method'=>'PUT','url'=>'admin/modules_questions/'.$modules->id,'id'=>'addmodules_questionsEditForm']) !!}
            <div class="form-body">
    ...
    <div class="portlet light col-md-6 bordered">
        ...
        <div class="portlet-body overflow" id="questionsContent" style="overflow: auto;">
            @include('auth.modules_questions2.questions.question_body', ['modules_questions' => $modules_questions])
        </div>
    </div>
</div>
@endsection
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('click', '.addNewQuestion', function (e) { //on add input button click
                ...
                $.ajax({
                    type: 'GET',
                    url: '{{URL("admin/modules_questions2")}}/' + type + '/' + id,
            ...
            $(document).on("click", ".remove_question", function (e) { //user click on remove text
                ...
                if(id == undefined) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{ URL('admin/modules_questions2') }}/"+module_id,
                        data: {
                            _method: 'DELETE',
                            _token: token,
                            id: id
            ...
            $(document).on('submit', '#addmodules_questionsEditForm', function (e) {
                ...
                $.ajax({
                    method: 'POST',
                    url: '{{URL("admin/modules_questions2")}}/' + id,
                    data: dataForm,
                    contentType: false,
                    processData: false,
                    success: function(response) {
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            module_questions.on('change', function () {
                ...
                if (moduleID) {
                    $.ajax({
                        url: "{{URL("admin/get_module_questions2")}}/" + moduleID,
                        type: "GET",
                        data: {_token: token},
                        dataType: "json",
                        success: function (data) {
                            if (data) {
                                if (data.length !== 0) {
                                    ...
                                    question_area.empty().append('<h3>No questions found</h3>');
                                }
                            } else {
                                question_area.hide()
                            }
            ...
            $('#importSelected').on('click', function (e) {
               ...
               // $('#importSelected').hide();
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{URL("admin/module_questions2_fetch")}}',
                    data: {
                        _token: token,
                        module: module_id,
                        ids: QuestionsIds
            ...
            $('#searchQuestion').on('keyup', function () {
                $("#checkAllQuestion").prop('checked', false);
                console.log(module_questions.val());
                var searchInput = $(this).val();
                $.ajax({
                    url: "{{URL("admin/search_module_question")}}",
                    type: "GET",
                    data: {_token: token, contains: searchInput, module_id: module_questions.val()},
                    dataType: "json",
                    success: function (data) {
                        console.log(data);
                        if (data != 'No search result') {
                            ...
                            $.each(data, function (key, value2) {
                                console.log(key);
                                console.log(value2.modules_questions_details);
                                if (value2.type === 'true_false') {
                                    str = JSON.stringify(value2);
                                    res = str.replace(/'/g, "&apos;");
                                    question_area.show().append('<div class="md-checkbox"><input data-json=\'' + res + '\' name="selectedQuestions"  data-type="' + value2.type + '" data-name="' + value2.name_ar + '" type="checkbox" id="checkbox-' + value2.id + '" value="' + value2.id + '" class="md-check all"><label for="checkbox-' + value2.id + '"><span class="inc"></span><span class="check"></span><span class="box"></span> ' + value2.name_ar + '</label><i style="color: #f0ad4e; margin-left: 5px;" class="fa fa-check"></i> </div>');
