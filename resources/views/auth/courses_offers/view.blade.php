...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.courses_offers') }}</span>
            </li>
        </ul>
    ...
    <!-- END PAGE BAR -->
    <!-- BEGIN PAGE TITLE-->
    <h1 class="page-title"> {{ Lang::get('main.courses_offers') }}
...
<style>
    ...
    @if(PerUser('courses_offers_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/courses_offers/create') }}" id="sample_editable_1_new" class="btn green">
    ...
    @endif
    @if(PerUser('courses_offers_view'))
        <div class="row">
            <div class="portlet light bordered">
                <div class="portlet-title">
                    <div class="caption font-dark">
                        <i class="icon-users font-dark"></i>
                        <span class="caption-subject bold uppercase">{{ Lang::get('main.courses_offers') }}</span>
                    </div>
                    <div class="tools"></div>
                </div>
                <div class="portlet-body">
                    <table style=" table-layout: fixed;width: 100%;" class="table table-striped table-bordered table-hover table-checkable" width="100%"
                        ...
                        <tr role="row" class="heading">
                            ...
                            <th>{{ Lang::get('main.egy_price') }}</th>
                            <th>{{ Lang::get('main.egy_sale_price') }}</th>
                            <th>{{ Lang::get('main.ksa_price') }}</th>
                            <th>{{ Lang::get('main.ksa_sale_price') }}</th>
                            <th>{{ Lang::get('main.expired_date') }}</th>
                        ...
                        <tr role="row" class="filter">
                            ...
                            <td></td>
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="egy_price">
                            </td>
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="egy_sale_price">
                            </td>
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="ksa_price">
                            </td>
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="ksa_sale_price">
                            </td>
                            <td>
                                <div class="input-group date fromToDate margin-bottom-5" data-date-format="yyyy-mm-dd">
                                    <input type="text" class="form-control form-filter input-sm"
                                           name="expired_time_from" placeholder="{{ Lang::get('main.from') }}">
                                ...
                                <div class="input-group date fromToDate" data-date-format="yyyy-mm-dd">
                                    <input type="text" class="form-control form-filter input-sm"
                                           name="expired_time_to" placeholder="{{ Lang::get('main.to') }}">
                                    <span class="input-group-btn">
                                            <button class="btn btn-sm default" type="button">
                                                <i class="fa fa-calendar"></i>
                                            </button>
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            @if(PerUser('courses_offers_publish'))
            $(document).on('change', '.changeStatues', function () {
                ...
                if (statues) {
                    $.ajax({
                        type: "POST",
                        url: "{{ URL('admin/courses_offers/publish') }}",
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/courses_offers_search') }}", // ajax source
                    },
            ...
            @if(PerUser('courses_offers_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.course_offer') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.course_offer') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/courses_offers') }}/" + deleted_id,
