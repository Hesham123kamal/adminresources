...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.courses_QandA') }}</span>
            </li>
        </ul>
...
@section('content')
    ...
    @if(PerUser('courses_QandA_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/courses_QandA/create') }}" id="sample_editable_1_new" class="btn green">
                ...
                <div class="portlet-body">
                    <table style=" table-layout: fixed;width: 100%;" class="table table-striped table-bordered table-hover dt-responsive" width="100%"
                        ...
                        <tr role="row" class="heading">
                            <th width="8%"> #</th>
                            <th width="10%">{{ Lang::get('main.course_id') }}</th>
                            <th width="10%">{{ Lang::get('main.section_id') }}</th>
                            <th width="10%">{{ Lang::get('main.user_name') }}</th>
                            <th width="20%">{{ Lang::get('main.question') }}</th>
                            <th width="20%">{{ Lang::get('main.answer') }}</th>
                            <th width="15%">{{ Lang::get('main.created_time') }}</th>
                            <th width="8%"></th>
                        </tr>
                        <tr role="row" class="filter">
                            ...
                            <td>
                                <select name="course_id" class="module_name sel2 form-control form-filter"
                                        id="course_id">
                            ...
                            <td>
                                <select name="section_id" class="module_name sel2 form-control form-filter"
                                        id="section_id">
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            @if(PerUser('courses_QandA_publish'))
            $(document).on('change', '.changeStatues', function () {
                ...
                if (statues) {
                    $.ajax({
                        type: "POST",
                        url: "{{ URL('admin/courses_QandA/publish') }}",
            ...
            grid.init({
                ...
                onDataLoad: function (grid) {
                    ...
                    $edit_drug.editable({
                        ...
                        url: "{{ URL('admin/courses_QandA') }}/" + $edit_drug_status_id + '/editsuccessstoriestatus',
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/courses_QandA_search') }}", // ajax source
                    },
            ...
            $(document).on('click', '.confirm_deletion', function (event) {
                ...
                $.ajax(
                    {
                        url: "{{ URL('admin/courses_QandA') }}/" + drug_id,
            ...
            @if(PerUser('courses_QandA_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.courses_QandA') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.courses_QandA') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/courses_QandA') }}/" + deleted_id,
