    ...
    <div class="row">
        <div class="portlet light bordered">
            ...
            <div class="portlet-body">
                {!! Form::open(['method'=>'PUT','url'=>'admin/books_categories/'.$book_category->id,'class'=>"form-horizontal"]) !!}
                <div class="form-body">
                ...
                <div class="form-group col-lg-9">
                    ...
                    <select id="category" name="category" class="module_name sel2 form-control form-filter">
                        ...
                        @foreach($categories as $id=>$name)
                            <option @if($book_category->category_id==$id)  selected="selected"
                ...
                <div class="form-group col-lg-3 text-center" style="margin-top:25px;">
                    <input type="checkbox" class="make-switch" name="published" value="yes"
                           @if($book_category->published=="yes") checked @endif data-size="small" data-on-color="success"
                ...
                <div class="form-group col-lg-12">
                    ...
                    <select id="sub_category" name="sub_category" class="module_name sel2 form-control form-filter">
                        ...
                        @foreach($sub_categories as $id=>$name)
                            <option @if($book_category->sup_category_id==$id)  selected="selected"
                                    @endif value="{{$id}}">{{$name}}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group col-lg-12">
                    ...
                    <select name="book" class="module_name sel2 form-control form-filter">
                        ...
                        @foreach($books as $id=>$name)
                            <option @if($book_category->book_id==$id)  selected="selected"
                                    @endif value="{{$id}}">{{$name}}</option>
                        @endforeach
                    </select>
                </div>
