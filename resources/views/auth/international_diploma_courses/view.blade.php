...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.international_diploma_courses') }}</span>
            </li>
        </ul>
...
<style>
    ...
    @if(PerUser('international_diploma_courses_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/international_diploma_courses/create') }}" id="sample_editable_1_new" class="btn green">
    ...
    @endif
    @if(PerUser('international_diploma_courses_view'))
        <div class="row">
                ...
                <div class="portlet-body">
                    <table style=" table-layout: fixed;width: 100%;" class="table table-striped table-bordered table-hover table-checkable" width="100%"
                        ...
                        <tr role="row" class="heading">
                            ...
                            <th>{{ Lang::get('main.related_course') }}</th>
                            <th>{{ Lang::get('main.sort') }}</th>
                            <th>{{ Lang::get('main.order') }}</th>
{{--                            <th>{{ Lang::get('main.published') }}</th>--}}
                        ...
                        <tr role="row" class="filter">
                            ...
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="order_field">
                            </td>
                            {{--<td></td>--}}
                            <td>
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            {{--@if(PerUser('international_diploma_courses_publish'))--}}
            {{--$(document).on('change', '.changeStatues', function () {--}}
                ...
                {{--if (statues) {--}}
                    {{--$.ajax({--}}
                        {{--type: "POST",--}}
                        {{--url: "{{ URL('admin/international_diploma_courses/publish') }}",--}}
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/international_diploma_courses_search') }}", // ajax source
                    },
            ...
            @if(PerUser('international_diploma_courses_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.international_diploma_courses') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.international_diploma_course') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/international_diploma_courses') }}/" + deleted_id,
