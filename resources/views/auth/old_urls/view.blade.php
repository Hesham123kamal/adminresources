...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.old_urls') }}</span>
            </li>
        </ul>
...
<style>
    ...
    @if(PerUser('old_urls_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/old_urls/create') }}" id="sample_editable_1_new" class="btn green">
                ...
                <div class="portlet-body">
                    <table style=" table-layout: fixed;width: 100%;" class="table table-striped table-bordered table-hover table-checkable" width="100%"
                        ...
                        <tr role="row" class="heading">
                            <th> #</th>
                            <th>{{ Lang::get('main.table_name') }}</th>
                            <th>{{ Lang::get('main.old_url') }}</th>
                            <th>{{ Lang::get('main.new_url') }}</th>
                        ...
                        <tr role="row" class="filter">
                            ...
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="table_name">
                            </td>
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="old_url">
                            </td>
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="new_url">
                            </td>
                            <td>
        ...
        $(document).ready(function () {
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/old_urls_search') }}", // ajax source
                    },
            ...
            @if(PerUser('old_urls_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.old_url') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.old_url') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/old_urls') }}/" + deleted_id,
