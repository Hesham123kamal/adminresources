...
@section('content')
    ...
    @if(PerUser('books_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/books/create') }}" id="sample_editable_1_new" class="btn green">
                ...
                <div class="portlet-body">
                    <table style=" table-layout: fixed;width: 100%;" class="table table-striped table-bordered table-hover dt-responsive" width="100%"
                        ...
                        <tr role="row" class="heading">
                            ...
                            <th width="10%">{{ Lang::get('main.title_en') }}</th>
                            <th width="10%">{{ Lang::get('main.author') }}</th>
                            <th width="20%">{{ Lang::get('main.book') }}</th>
                            <th width="20%">{{ Lang::get('main.url') }}</th>
                            <th width="20%">{{ Lang::get('main.rating') }}</th>
                            <th width="20%">{{ Lang::get('main.rating_count') }}</th>
                        ...
                        <tr role="row" class="filter">
                            ...
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="title_en">
                            </td>
                            <td>
                            ...
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="book">
                            </td>
                            <td>
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            grid.init({
                ...
                onDataLoad: function (grid) {
                    ...
                    $edit_drug.editable({
                        ...
                        url: "{{ URL('admin/books') }}/" + $edit_drug_status_id + '/editsuccessstoriestatus',
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/books_search') }}", // ajax source
                    },
            ...
            @if(PerUser('books_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.books') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.books') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/books') }}/" + deleted_id,
