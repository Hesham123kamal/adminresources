...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.session_courses_views') }}</span>
            </li>
        </ul>
...
@if(PerUser('session_courses_views_add'))
    <div class="row">
        <div class="col-md-12">
            <div class="btn-group pull-right">
                <a href="{{ URL('admin/session_courses_views/create') }}" id="sample_editable_1_new" class="btn green">
...
@endif
    @if(PerUser('session_courses_views_view'))
        <div class="row">
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            @if(PerUser('session_courses_views_publish'))
            $(document).on('change', '.changeStatues', function () {
                ...
                if (statues) {
                    $.ajax({
                        type: "POST",
                        url: "{{ URL('admin/session_courses_views/publish') }}",
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/session_courses_views_search') }}", // ajax source
                    },
            ...
            @if(PerUser('session_courses_views_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.session_course_view') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.session_course_view') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/session_courses_views') }}/" + deleted_id,
