...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.our_partners') }}</span>
            </li>
        </ul>
...
@section('content')
    ...
    @if(PerUser('our_partner_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/our_partner/create') }}" id="sample_editable_1_new" class="btn green">
                        {{ Lang::get('main.add') }}
    ...
    @endif
    @if(PerUser('our_partner_view'))
        <div class="row">
            <div class="portlet light bordered">
                <div class="portlet-title">
                    <div class="caption font-dark">
                        <i class="icon-users font-dark"></i>
                        <span class="caption-subject bold uppercase">{{ Lang::get('main.our_partner') }}</span>
                    </div>
                    <div class="tools"></div>
                </div>
                <div class="portlet-body">
                    <table class="table table-striped table-bordered table-hover dt-responsive" width="100%"
                        ...
                        <tr role="row" class="heading">
                            ...
                            <th>{{ Lang::get('main.flag') }}</th>
                            <th>{{ Lang::get('main.phone') }}</th>
                            <th>{{ Lang::get('main.responsible_name') }}</th>
                        ...
                        <tr role="row" class="filter">
                            ...
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="flag">
                            </td>
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="phone">
                            </td>
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="responsible_name">
                            </td>
                            <td>
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            @if(PerUser('our_partner_publish'))
            $(document).on('change', '.changeStatues', function () {
                ...
                if (statues) {
                    $.ajax({
                        type: "POST",
                        url: "{{ URL('admin/our_partner/publish') }}",
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/our_partner_search') }}", // ajax source
                    },
            ...
            @if(PerUser('our_partner_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.our_partner') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.our_partner') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/our_partner') }}/" + deleted_id,
