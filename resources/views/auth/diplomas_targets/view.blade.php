...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.diplomas_targets') }}</span>
            </li>
        </ul>
...
<style>
    ...
    @if(PerUser('diplomas_targets_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/diplomas_targets/create') }}" id="sample_editable_1_new" class="btn green">
    ...
    @endif
    @if(PerUser('diplomas_targets_view'))
        <div class="row">
...
{{--                            <th>{{ Lang::get('main.published') }}</th>--}}
                        ...
                        <tr role="row" class="filter">
                            ...
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="target">
                            </td>
                            <td>
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            {{--@if(PerUser('diplomas_targets_publish'))--}}
            {{--$(document).on('change', '.changeStatues', function () {--}}
                ...
                {{--if (statues) {--}}
                    {{--$.ajax({--}}
                        {{--type: "POST",--}}
                        {{--url: "{{ URL('admin/diplomas_targets/publish') }}",--}}
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/diplomas_targets_search') }}", // ajax source
                    },
            ...
            @if(PerUser('diplomas_targets_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.diploma_target') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.diploma_target') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/diplomas_targets') }}/" + deleted_id,
