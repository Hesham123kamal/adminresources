...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.webinars') }}</span>
            </li>
        </ul>
...
@section('content')
    ...
    @if(PerUser('webinars_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/webinars/create') }}" id="sample_editable_1_new" class="btn green">
                            ...
                            <td>
                                ...
                                <select name="parent_id" class="module_name sel2 form-control form-filter" id="parent_id">
                                    ...
                                    @foreach($parent_webinars as $parent_webinar)
                                        <option value="{{$parent_webinar->id}}">{{$parent_webinar->name}}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td>
        ...
        $(document).ready(function () {
            ...
            $(document).on('change', '.changeStatues', function () {
                ...
                if (statues) {
                    $.ajax({
                        type: "POST",
                        url: "{{ URL('admin/webinars/activation') }}",
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/webinars_search') }}", // ajax source
                    },
            ...
            $(document).on('click', '.confirm_deletion', function (event) {
                ...
                $.ajax(
                    {
                        url: "{{ URL('admin/webinars') }}/" + drug_id,
            ...
            @if(PerUser('webinars_convert'))
            $(document).on('click', '.convertToCourse', function (event) {
                var url = $(this).attr("data-url");
                var name = $(this).attr("data-name");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.convert').lang::get('main.webinar') }} '+name,
                    message: '{{ Lang::get('main.convert_this').lang::get('main.webinar') }} '+name+' ?',
                    buttons: [
                        {
            ...
            @endif
            @if(PerUser('webinars_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.webinars') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.webinars') }} ?',
                    buttons: [
                        {
                            label: '{{ Lang::get('main.yes') }}',
                            cssClass: 'btn-primary',
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/webinars') }}/" + deleted_id,
