...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.sub_categories') }}</span>
            </li>
        </ul>
...
<style>
    ...
    @if(PerUser('sub_categories_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/sub_categories/create') }}" id="sample_editable_1_new" class="btn green">
    ...
    @endif
    @if(PerUser('sub_categories_view'))
        <div class="row">
                ...
                <div class="portlet-body">
                    <table style=" table-layout: fixed;width: 100%;" class="table table-striped table-bordered table-hover table-checkable" width="100%"
                        ...
                        <tr role="row" class="heading">
                            ...
                            {{--<th>{{ Lang::get('main.courses_count') }}</th>--}}
                            {{--<th>{{ Lang::get('main.webinar_count') }}</th>--}}
                            {{--<th>{{ Lang::get('main.webinar_offline_count') }}</th>--}}
                            {{--<th>{{ Lang::get('main.successstories_count') }}</th>--}}
                            {{--<th>{{ Lang::get('main.books_count') }}</th>--}}
                        ...
                        <tr role="row" class="filter">
                            ...
                            {{--<td>--}}
                                {{--<input type="text" class="form-control form-filter input-sm" name="courses_count">--}}
                            {{--</td>--}}
                            {{--<td>--}}
                                {{--<input type="text" class="form-control form-filter input-sm" name="webinar_count">--}}
                            {{--</td>--}}
                            {{--<td>--}}
                                {{--<input type="text" class="form-control form-filter input-sm" name="webinar_offline_count">--}}
                            {{--</td>--}}
                            {{--<td>--}}
                                {{--<input type="text" class="form-control form-filter input-sm" name="successstories_count">--}}
                            {{--</td>--}}
                            {{--<td>--}}
                                {{--<input type="text" class="form-control form-filter input-sm" name="books_count">--}}
                            {{--</td>--}}
                            <td>
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            @if(PerUser('sub_categories_publish'))
            $(document).on('change', '.changeStatues', function () {
                ...
                if (statues) {
                    $.ajax({
                        type: "POST",
                        url: "{{ URL('admin/sub_categories/publish') }}",
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/sub_categories_search') }}", // ajax source
                    },
            ...
            @if(PerUser('sub_categories_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.sub_category') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.sub_category') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/sub_categories') }}/" + deleted_id,
