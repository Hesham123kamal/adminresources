...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.tpay_price_book') }}</span>
            </li>
        </ul>
...
<style>
    ...
    @if(PerUser('tpay_price_book_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/tpay_price_book/create') }}" id="sample_editable_1_new" class="btn green">
    ...
    @endif
    @if(PerUser('tpay_price_book_view'))
        <div class="row">
                ...
                <div class="portlet-body">
                    <table style=" table-layout: fixed;width: 100%;" class="table table-striped table-bordered table-hover table-checkable" width="100%"
                        ...
                        <tr role="row" class="heading">
                            ...
                            <th>{{ Lang::get('main.sku') }}</th>
                            <th>{{ Lang::get('main.operator_code') }}</th>
                            <th>{{ Lang::get('main.operator_name') }}</th>
                            <th>{{ Lang::get('main.currency_symbol') }}</th>
                        ...
                        <tr role="row" class="filter">
                            ...
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="operator_code">
                            </td>
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="operator_name">
                            </td>
                            <td>
                                <input type="text" class="form-control form-filter input-sm" name="currency_symbol">
                            </td>
                            <td>
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            {{--@if(PerUser('tpay_price_book_publish'))--}}
            {{--$(document).on('change', '.changeStatues', function () {--}}
                ...
                {{--if (statues) {--}}
                    {{--$.ajax({--}}
                        {{--type: "POST",--}}
                        {{--url: "{{ URL('admin/tpay_price_book/publish') }}",--}}
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/tpay_price_book_search') }}", // ajax source
                    },
            ...
            {{--$(document).on('click', '.confirm_deletion', function (event) {--}}
                ...
                {{--$.ajax(--}}
                    {{--{--}}
                        {{--url: "{{ URL('admin/static_pages') }}/" + drug_id,--}}
            ...
            {{--});--}}
            @if(PerUser('tpay_price_book_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.tpay_price_book') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.tpay_price_book') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/tpay_price_book') }}/" + deleted_id,
