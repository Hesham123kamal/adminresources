...
@section('contentHeader')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <span>{{ Lang::get('main.mba_courses_user_plan') }}</span>
            </li>
        </ul>
...
@section('content')
    ...
    @if(PerUser('mba_courses_user_plan_add'))
        <div class="row">
            <div class="col-md-12">
                <div class="btn-group pull-right">
                    <a href="{{ URL('admin/mba_courses_user_plan/create') }}" id="sample_editable_1_new" class="btn green">
    ...
    @endif
    @if(PerUser('mba_courses_user_plan_view'))
        <div class="row">
...
@section('scriptCode')
    ...
    <script>
        $(document).ready(function () {
            ...
            @if(PerUser('mba_courses_user_plan_active'))
            $(document).on('change', '.changeStatues', function () {
                ...
                if (statues) {
                    $.ajax({
                        type: "POST",
                        url: "{{ URL('admin/mba_courses_user_plan/activation') }}",
            ...
            grid.init({
                ...
                dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
                    ...
                    "ajax": {
                        "url": "{{ URL('admin/mba_courses_user_plan_search') }}", // ajax source
                    },
            ...
            $(document).on('click', '.confirm_deletion', function (event) {
                ...
                $.ajax(
                    {
                        url: "{{ URL('admin/mba_courses_user_plan') }}/" + drug_id,
            ...
            @if(PerUser('mba_courses_user_plan_delete'))
            $(document).on('click', '.delete_this', function (event) {

                var deleted_id = $(this).attr("data-id");
                event.preventDefault();
                BootstrapDialog.show({
                    title: '{{ Lang::get('main.delete').lang::get('main.mba_courses_user_plan') }}',
                    message: '{{ Lang::get('main.delete_this').lang::get('main.mba_courses_user_plan') }} ?',
                    buttons: [
                        {
                            ...
                            action: function (dialogItself) {
                                $.ajax({
                                    type: "DELETE",
                                    url: "{{ URL('admin/mba_courses_user_plan') }}/" + deleted_id,
