...
use App\SessionSuccesstoriesViews;
...
class SessionSuccesstoriesViewsController extends Controller
{
    /**
    ...
    {
        return view('auth.session_successtories_views.view');
    }
    ...
    function search(Request $request)
    {
        ...
        $data = $request->input();
        $ssvs = SessionSuccesstoriesViews::leftjoin('users','users.id','=','session_successtories_views.user_id')
            ->leftjoin('successtories','successtories.id','=','session_successtories_views.successtory_id')
            ->select('session_successtories_views.*','successtories.name as successtory_name','users.Email as user_email');
        if (isset($data['id']) && !empty($data['id'])) {
            $id = $data['id'];
            $ssvs = $ssvs->where('session_successtories_views.id', '=', $id);
        }
        ...
        if (isset($data['count']) && !empty($data['count'])) {
            $count = $data['count'];
            $ssvs = $ssvs->where('session_successtories_views.count', '=', $count);
        }
        ...
        if (isset($data['created_time_from']) && !empty($data['created_time_from']) && isset($data['created_time_to']) && !empty($data['created_time_to'])) {
            ...
            $ssvs = $ssvs->whereBetween('session_successtories_views.createdtime', [$created_time_from .' 00:00:00', $created_time_to.' 23:59:59']);
        }
        ...
        $columnName = 'session_successtories_views.id';
        switch ($data['order'][0]['column']) {
            ...
            case 2:
                ...
                break;
            case 3:
                $columnName = 'session_successtories_views.count';
                break;
            case 4:
                $columnName = 'session_successtories_views.createdtime';
                break;
        }
        ...
        if ($search) {
            $ssvs = $ssvs->where(function ($q) use ($search) {
                $q->where('session_successtories_views.id', '=', $search)
                    ->orWhere('session_successtories_views.count', '=', $search)
            ...
            $records["data"][] = [
                ...
                '<td class="text-center">
                                <div class="checkbox-nice checkbox-inline">
                                    <input data-id="' . $ssv->id . '" type="checkbox" ' . ((!PerUser('session_successtories_views_publish')) ? 'disabled="disabled"' : '') . ' ' . ((PerUser('session_successtories_views_publish')) ? 'class="changeStatues"' : '') . ' ' . (($ssv->published=="yes") ? 'checked="checked"' : '') . ' ">
                ...
                '<div class="btn-group text-center" id="single-order-' . $ssv->id . '">
                                    ...
                                    ' . ((PerUser('session_successtories_views_edit')) ? '<li>
                                            <a href="' . URL('admin/session_successtories_views/' . $ssv->id . '/edit') . '">
                                                <i class="fa fa-comments-o"></i> ' . Lang::get('main.edit') . '
                                            </a>
                                        </li>' : '') . '
                                    ' . ((PerUser('session_successtories_views_delete')) ? '<li>
    ...
    public function create()
    {
        ...
        return view('auth.session_successtories_views.add',compact('successtories'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $successtory_view = new SessionSuccesstoriesViews();
            $successtory_view->successtory_id = $data['successtory'];
            $successtory_view->user_id = $user;
            ...
            if ($successtory_view->save()) {
                Session::flash('success', Lang::get('main.insert') . Lang::get('main.session_successtory_view'));
                return Redirect::to('admin/session_successtories_views/create');
            }
        }
    }
    ...
    {
        $successtory_view = SessionSuccesstoriesViews::findOrFail($id);
        $successtories=Successstories::pluck('name', 'id');
        $user=NormalUser::where('id', $successtory_view->user_id)->first();
        $user=($user!==null)?$user->Email:'';
        return view('auth.session_successtories_views.edit', compact('successtory_view','successtories','user'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($successtory_view->save()) {
                Session::flash('success', Lang::get('main.update') . Lang::get('main.session_successtory_view'));
                return Redirect::to("admin/session_successtories_views/$successtory_view->id/edit");
            }
        }
    }
